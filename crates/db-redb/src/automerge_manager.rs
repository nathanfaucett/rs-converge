...
use dashmap::DashMap;
use db_automerge::AutomergeBTree;
...
trait RedbAutomergeBTreeManagerValue: core::any::Any + MaybeSend + MaybeSync + 'static {}
impl<T> RedbAutomergeBTreeManagerValue for T where
  T: core::any::Any + MaybeSend + MaybeSync + 'static
{
}

pub struct RedbAutomergeBTreeManager {
  db: Arc<redb::Database>,
  inner: DashMap<String, Box<dyn RedbAutomergeBTreeManagerValue>>,
  names: DashMap<String, &'static str>,
}

impl RedbAutomergeBTreeManager {
...
impl BTreeManager for RedbAutomergeBTreeManager {
  type BTree<K, V>
    = AutomergeBTree<RedbBTree<K, V>>
  where
    K: BTreeKey,
    V: BTreeValue;
    ...
    D: BTreeDefinition,
  {
    ...
    let boxed: Box<dyn RedbAutomergeBTreeManagerValue> = Box::new(btree.clone());
