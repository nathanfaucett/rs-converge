...
use db_engine::{Checkpoint, Engine, EngineError, Kernel, RowCodec};
use thiserror::Error;
...
async fn receive_frontier<T>(transport: &mut T) -> Result<db_engine::Frontier, SyncError<T::Error>>
where
    T: SyncTransport,
    T::Error: core::fmt::Display,
{
...
async fn send_transfer<K, R, T>(
    engine: &Engine<K, R>,
    transport: &mut T,
    envelopes: Vec<db_engine::TransactionEnvelope>,
    batch_size: usize,
    ...
    T::Error: core::fmt::Display,
{
    ...
    for batch in envelopes.chunks(batch_size) {
        let bytes = batch
            .iter()
            .map(db_engine::TransactionEnvelope::encode)
