...
use crate::{QueryResultColumn, Row, TableSchema};
...
pub trait FromRow: DeserializeOwned {
  fn from_row(schema: &TableSchema, row: &Row) -> Result<Self, RowDeserializeError> {
    let cols: Vec<QueryResultColumn> = schema
      .columns
      .iter()
      .enumerate()
      .map(|(i, c)| QueryResultColumn {
        name: c.name.clone(),
        source_table: None,
  ...
  fn from_named_row(columns: &[QueryResultColumn], row: &Row) -> Result<Self, RowDeserializeError> {
